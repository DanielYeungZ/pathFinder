...
class BuildingRoutesTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.app.register_blueprint(building_bp)
    ...
    def test_create_building_success(self):
        response = self.client.post('/building', headers={'Authorization': self.valid_token},
    ...
    def test_create_building_missing_token(self):
        response = self.client.post('/building', json={'name': 'Test Building'})
        self.assertEqual(response.status_code, 401)
        self.assertIn('Token is missing', response.json['message'])

    def test_create_building_invalid_token(self):
        response = self.client.post('/building', headers={'Authorization': 'invalid_token'},
                                    json={'name': 'Test Building'})
        self.assertEqual(response.status_code, 401)
